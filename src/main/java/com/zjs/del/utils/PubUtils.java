...
public class PubUtils {
	 ...
	 * 匹配正浮点数
	 */
	public static final String REGX_POSITIVE_RATIONAL_NUMBERS = "^((([0-9]+\\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\\.[0-9]+)|([0-9]*[1-9][0-9]*)))$";
	...
	/**
	 * 是否是正实数 包含浮点数，如果是浮点数将返回true例如：125.2500或125 都将返回true
	 *
	 * @since 1.0
	 * @author 刘晓辉
	 * @time 2010-9-19 下午03:31:49
	 */
	public static Boolean isPositive(String inputStr) {
		if (!PubUtils.isNull(inputStr)) {
			return inputStr.matches(REGX_POSITIVE_RATIONAL_NUMBERS);
		}
		return false;
	}
		...
		if (!PubUtils.isNull(inputStr)) {
			...
			for (int i = 0; i < inputStr.length(); i++) {
				try {
					...
					LOGGER.error("sbcTodbcChange,发生异常:" + e);
				}
				...
				if (b[2] == -1) {
					...
					try {
						...
						LOGGER.error("sbcTodbcChange方法,发生异常:" + e);
					}
				} else {
